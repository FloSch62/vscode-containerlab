/**
 * TopoViewerProvider - Main orchestrator for React TopoViewer
 *
...
export class TopoViewer {
  ...
  private readonly viewType = 'topoViewer';
    ...
    try {
      ...
      if (missingIds.length === 1 && orphanAnnotations.length > 0) {
        ...
        if (candidate) {
          ...
          log.info(`[TopoViewer] Migrated annotation id from ${oldId} to ${newId} after YAML rename`);
          return true;
        }
      }
    } catch (err) {
      log.warn(`[TopoViewer] Failed to reconcile annotations on rename: ${err}`);
    }
  ...
  public async refreshAfterExternalCommand(
    ...
    try {
      ...
      log.info(`[TopoViewer] Refreshed after ${newDeploymentState === 'deployed' ? 'deploy' : 'destroy'}`);
      return true;
    } catch (err) {
      log.error(`[TopoViewer] Failed to refresh after external command: ${err}`);
      return false;
    }
  }
  ...
  private async notifyWebviewModeChanged(): Promise<void> {
    ...
    log.info(`[TopoViewer] Mode changed to: ${mode}`);
  }
  ...
  public async refreshLinkStatesFromInspect(
    ...
    try {
      ...
      log.error(`[TopoViewer] Failed to refresh link states: ${err}`);
    }
  }
}
 ...
 */
export class TopoViewerProvider {
  private static instance: TopoViewerProvider | undefined;
  private viewers: Map<string, TopoViewer> = new Map();
  ...
  public static getInstance(context: vscode.ExtensionContext): TopoViewerProvider {
    if (!TopoViewerProvider.instance) {
      TopoViewerProvider.instance = new TopoViewerProvider(context);
    }
    return TopoViewerProvider.instance;
  }
  ...
  public async openViewer(
    ...
    // Create new viewer
    const viewer = new TopoViewer(this.context);
